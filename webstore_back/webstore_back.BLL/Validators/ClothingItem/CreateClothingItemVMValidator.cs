...
{
    public class CreateClothingItemVMValidator : AbstractValidator<CreateClothingItemVM>
    {
        public CreateClothingItemVMValidator()
        {
            RuleFor(m => m.Name)
            ...
            RuleFor(m => m.CategoryId)
                .NotEmpty().WithMessage("Вкажіть категорію.");
        }
    }
}